# Temperatura do Ar (°C) - Máximo Semanal
...
dados_RS <- read_excel("07.1. PA - BGEV - Temperatura Máxima do Ar.xlsx") %>% arrange(ano,semana)
...
ggplot(dados_RS, aes(y = `TEMPERATURA DO AR - BULBO SECO (C)`, 
  ...
  labs(title = " ",
       y = "Temperatura Máxima Semanal (°C) \n",
...
curvas <- data.frame(
  x = rep(sequencia, 1),
  densidade = c(
    dbgev(sequencia, mu = 30, sigma = 5, xi = -0.4, delta = 0)
  ),
  distrib = factor(rep(c("μ = 30,0\n\nσ = 5,00\n\nξ = -0,40\n\nδ = 0,00\n"), each = 168))
)
...
ggplot() +
  ...
  geom_histogram(aes(x = dados_RS$`TEMPERATURA DO AR - BULBO SECO (C)`, y = after_stat(density)), 
                 binwidth = 2.5, fill = "#325ea8", color = "black") +
...
serie_temporal %>% autoplot() + 
  ...
  xlab("\n Tempo (meses)") +
  ylab("Temperatura Máxima Semanal (°C) \n") +
...
acf(Residuos[3:62], lag.max = 12*5,  main = "Gráfico ACF")
pacf(Residuos[3:62], lag.max = 12*5,  main = "Gráfico PACF")
par(mfrow=c(1,1))
hist(Residuos)
...
ar_1 <- NA    # termos autorregressivos
...
novo_arquivo_excel <- "dados_saida(0,1).xlsx"
